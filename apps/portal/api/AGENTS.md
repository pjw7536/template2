...
## Scope
These rules apply to `apps/portal/api/**`.

## 작업별 스킬
- 아래 스킬 경로는 저장소 루트 기준이다. 현재 작업에 해당하는 스킬만 읽는다.
- 새 도메인 scaffold: `.codex/skills/create-django-feature/SKILL.md`.
- 업무 로직 테스트·migration: `.codex/skills/django-test-migration-flow/SKILL.md`.
- 도메인 경계 검사: `.codex/skills/backend-boundary-audit/SKILL.md`.
- Python 코드의 설명·주석 작성: `.codex/skills/write-commented-python/SKILL.md`.

## Domain App Boundary
- Domain app path: `apps/portal/api/api/<feature>` (`api.<feature>`)
- Data movement table apps are the only nested domain app exception:
  - Namespace path: `apps/portal/api/api/data_movement/<table_name>` (`api.data_movement.<table_name>`)
  ...
  - Shared ingestion helpers for these apps live under `apps/portal/api/api/data_movement/common`.
...
- Shared/infrastructure packages only:
  - `apps/portal/api/api/common`
  - `apps/portal/api/api/data_movement/common`
  - `apps/portal/api/api/auth`
  - `apps/portal/api/api/rag`
  - `apps/portal/api/api/management`
...
- Global routing only in `apps/portal/api/api/urls.py` as include registry.
...
- After backend domain boundary/import/view/selector changes, run or recommend `make audit-api-boundary`.
