# =============================================================================
# 모듈 설명: provider별 OIDC 세션 인증 서비스 흐름을 제공합니다.
# - 주요 대상: 인증 설정, 로그인 URL 생성, callback 검증, 로그아웃 URL
# - 불변 조건: 외부 API 응답 형식과 세션 계약은 view 계층에서 그대로 보존합니다.
# =============================================================================

"""ADFS id_token과 Keycloak code flow를 지원하는 세션 인증 서비스.
...
class OidcCallbackResult:
    ...
    user: Any = None
    raw_id_token: Optional[str] = None
...
def auth_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "responseMode": "query" if is_keycloak else "form_post",
        "responseType": "code" if is_keycloak else "id_token",
...
def auth_login(*, requested_target: Optional[str], request: HttpRequest) -> OidcLoginResult:
    """provider에 맞는 authorize URL과 세션 nonce를 준비합니다.
    ...
    if getattr(settings, "OIDC_PROVIDER", "adfs") == "keycloak":
        return OidcLoginResult(
            authorize_url=keycloak_oidc.build_authorize_url(
                request=request,
                state=state,
                nonce=nonce,
            )
...
def auth_callback(
    *,
    ...
    """provider별 callback 값을 검증하고 세션 로그인 대상 사용자를 반환합니다.

    입력:
    - request: Django HttpRequest
    - raw_id_token: ADFS form_post로 전달된 id_token
    - code: Keycloak query로 전달된 authorization code
    - state: callback으로 전달된 state
    ...
    if getattr(settings, "OIDC_PROVIDER", "adfs") == "keycloak":
        if not code:
            return OidcCallbackResult(target=target, error_code="missing_code")
        try:
            token_set = keycloak_oidc.exchange_code(request=request, code=code)
        except keycloak_oidc.KeycloakOidcError:
            return OidcCallbackResult(
                target=target,
                bad_request_message="keycloak token exchange failed",
            )
        raw_id_token = str(token_set.get("id_token") or "")

    if not raw_id_token:
        return OidcCallbackResult(target=target, error_code="missing_id_token")
...
def auth_logout(*, request: HttpRequest) -> str:
    """provider에 맞는 IdP 로그아웃 URL을 반환합니다.
    ...
    부작용:
    - Keycloak provider에서는 세션의 id_token 제거
