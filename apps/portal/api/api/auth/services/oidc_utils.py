...
def _load_configured_public_certificate() -> tuple[x509.Certificate | None, object | None]:
    """provider가 인증서 방식일 때만 공개 인증서와 키를 로드합니다.

    입력:
    - 없음(settings 사용)

    반환:
    - tuple[x509.Certificate | None, object | None]: 인증서와 공개키

    부작용:
    - ADFS provider에서는 인증서 파일 읽기

    오류:
    - RuntimeError: ADFS 인증서 파일을 읽을 수 없는 경우
    """

    if getattr(settings, "OIDC_PROVIDER", "adfs") == "keycloak":
        return None, None
    certificate = load_x509_cert(CER_PATH)
    return certificate, certificate.public_key()


PUB_CERT, PUB_KEY = _load_configured_public_certificate()
