...
# - 불변 조건: SPA와 Airflow HTTP 계약은 camelCase를 사용합니다.
# =============================================================================
...
- 가정/불변 조건: HTTP 입력은 camelCase 하나만 허용함
"""
...
class ExternalAffiliationRecordSerializer(StrictSerializer):
    """외부 DB에서 전달되는 사용자 예측 소속 레코드 입력 스키마."""

    knoxId = StrictTextField(source="knox_id", max_length=150)
    ...
    userSdwtProd = StrictTextField(source="user_sdwt_prod", max_length=64)
...
class ExternalAffiliationSyncSerializer(StrictSerializer):
    """외부 예측 소속 동기화 요청 스키마."""

    records = ExternalAffiliationRecordSerializer(many=True)


class AffiliationReconfirmResponseSerializer(StrictSerializer):
    """소속 재확인 응답 입력 스키마."""

    accepted = serializers.BooleanField()
    userSdwtProd = StrictTextField(
        source="user_sdwt_prod",
        max_length=64,
        required=False,
        allow_blank=True,
    )
