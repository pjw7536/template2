...
새 DB에 migration을 적용하고 Keycloak client를 준비합니다. 사용자는 최초 로그인에서
`userid`(EPID)로 생성됩니다. 사번이나 Knox ID로 자동 연결하지 않습니다.
앱 접근은 Keycloak Portal client 역할만으로 판단합니다. 내부 사용자는 조직 정책에 따라
`portal-members` 그룹에 가입시켜 `portal-all-apps`를 상속하고, 제한 사용자는 개별 앱 역할을 지정합니다.
`deptid`는 신원 정보이며 앱 권한을 부여하지 않습니다. SDWT 그룹의 viewer/user/admin은
각각 조회/수정/삭제 등급이며 소속만으로 접근할 수 없습니다.

소속·권한 수정은 Keycloak에서 수행하고 다음 로그인에 반영합니다. Portal은 조회만 제공합니다.
전체 조직 CSV나 사용자 사전 등록은 필요하지 않습니다. 운영 절차는
[Portal 배포 안내](../../deploy/portal/README.md#keycloak-권한과-새-db-시작)를 따릅니다.

로컬 Keycloak의 EPID `90000001`은 내부 사용자, `90000002`는 메일 viewer,
`90000003`은 외부 전체 앱 사용자, `90000004`는 전체 관리자, `90000005`는 무권한 사용자입니다.
로컬 테스트 비밀번호는 모두 `dummy-user-change-me`입니다. realm import는 기존 realm을
덮어쓰지 않으므로 기존 로컬 설치를 자동 삭제하지 않습니다. 새 fixture 검증은 별도 환경에서 수행합니다.
