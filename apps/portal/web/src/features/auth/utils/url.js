...
/**
 * target 파라미터에 사용할 절대 URL 생성
 * @param {string | undefined} targetPath - 이동하고 싶은 경로(절대/상대 허용)
 * @param {string | undefined} base - 기준이 될 절대 경로 (예: 프론트엔드 도메인)
 * @returns {string | undefined}
 */
export function buildTargetUrl(targetPath, base) {
...
/**
 * 로그인 URL에 target 파라미터를 붙여주는 함수
 * @param {string} url - 로그인 URL(절대/상대)
 * @param {string | undefined} targetUrl - target 파라미터로 전달할 절대 URL
 */
export function appendTargetParam(url, targetUrl) {
  ...
  try {
    ...
    return `${url}${sep}target=${encodeURIComponent(targetUrl)}`
  }
}
