...
def _split_sensor(value: str) -> tuple[str, str]:
    """sensor 문자열 끝의 ch_step@iteration 정보를 분리합니다."""
...
def _build_step_maps(line_id: str) -> tuple[dict[str, list[str]], dict[str, list[str]], list[str]]:
    """라인 하위 model/step 디렉터리에서 hardspec.py와 같은 step option을 만듭니다."""
...
def _collect_recipe_context(*, line_id: str, step_seq: str) -> dict[str, Any]:
    ...
    try:
        ...
        db_warning = f"FDC model DB 조회 실패로 eqp_model 목록을 사용했습니다: {exc}"
    ...
    return {
        ...
        "warnings": [db_warning] if db_warning else [],
    }
...
def _load_min_max_data(paths: list[Path]) -> pd.DataFrame:
    """HDFS 통계 parquet에서 sensor별 추천 Spec min/max를 계산합니다."""
...
def get_hard_spec_meta(params: dict[str, Any]) -> dict[str, Any]:
    ...
    context = _collect_recipe_context(line_id=line_id, step_seq=selected_step_seq) if selected_step_seq else {
        "recipeIds": [],
        "fdcModels": [],
        "warnings": [],
    }
    selected_recipe_id = recipe_id or (context["recipeIds"][0] if context["recipeIds"] else "")
    if selected_recipe_id and selected_recipe_id != recipe_id:
        context = _collect_recipe_context(line_id=line_id, step_seq=selected_step_seq)
...
def get_hard_spec_recommendations(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if min_max_data.empty:
        return {"rows": [], "chartRows": [], "warnings": [*context["warnings"], "선택 조건에 맞는 통계 parquet 데이터가 없습니다."]}
    ...
    if min_max_data.empty:
        return {"rows": [], "chartRows": [], "warnings": [*context["warnings"], "선택 FDC model의 A/B priority 센서가 없습니다."]}

    hard_spec = _load_hard_spec(fdc_model=fdc_model, recipe_id=recipe_id, min_max_data=min_max_data)
    if hard_spec.empty:
        return {"rows": [], "chartRows": [], "warnings": [*context["warnings"], "선택 조건에 맞는 HARD_LIMIT 데이터가 없습니다."]}
    ...
    rows = []
    for index, row in result[["priority", "sensor_name", "ch_step", *NUMERIC_COLUMNS, "Spec격차"]].iterrows():
        item = {column: _safe_value(row[column]) for column in ["priority", "sensor_name", "ch_step", *NUMERIC_COLUMNS, "Spec격차"]}
