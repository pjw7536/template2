...
class AppLayoutTests(unittest.TestCase):
    ...
    def test_catalog_distinguishes_app_ownership_from_tools_and_runtime(self):
        """설치형 제품의 소스·미등록 로컬 폴더는 거부하고 생성물은 보존합니다."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            catalog = {'apps': {'keycloak': {'sourcePaths': [], 'deployPath': 'deploy/keycloak',
                                             'localPath': 'local/keycloak'}}}
            for name in ('deploy/keycloak', 'local/keycloak', 'apps/tooling/node_modules',
                         'local/shared/runtime', 'local/adfs_dummy', 'deploy/shared',
                         'deploy/keycloak/rendered', 'local/__pycache__'):
                (root / name).mkdir(parents=True)
            self.assertFalse(audit.check_catalog(root, catalog))
            for name in ('apps/keycloak', 'local/orphan', 'deploy/orphan'):
                (root / name).mkdir(parents=True)
            self.assertEqual(len(audit.check_catalog(root, catalog)), 3)
