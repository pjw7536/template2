...
Keycloak·Portal·Airflow·FTP·Monitoring·Headlamp는 공통 Kubernetes 정의를 사용합니다.
...
직접 개발하는 앱 소스는 `apps/portal`·`apps/airflow`, 검사 도구는 `apps/tooling`이 소유합니다.
로컬 Kustomize 집계는 `local/shared/k8s`이며 Portal·Keycloak·Headlamp·mock의 원본을 참조합니다.
`make k8s-render-local`로 집계와 앱별 원본을 확인합니다. 서버 선택 checkout에는 local이 필요하지 않습니다.
