...
> 2026-09-25: 아래의 고정 SDWT ID·조직 속성 상속·앱별 역할 묶음 설계는
> [SDWT 하위 그룹 기반 공통 권한 계획](keycloak-sdwt-group-authorization.md)으로 대체한다.
> 최종 계약은 SDWT 이름을 유지한 `/{SDWT}/admin|user|viewer` 그룹과 별도 소속 속성이다.
> 이 문서는 과거 조사 기록이며, 아래의 미확정 정책·실행 단계는 현재 구현 지시가 아니다.
