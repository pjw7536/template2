# Activity 백엔드 로직 (feature: activity)
...
1. 인증 확인.
2. 권한 확인:
   - `activity.view_activitylog` 또는 레거시 `api.view_activitylog` 권한 필요.
3. `limit` 파싱 (기본 50, 최소 1, 최대 200).
4. `get_recent_activity_payload` → `get_recent_activity_logs` 조회.
5. `serialize_activity_log`로 직렬화:
   - `metadata.remote_addr`가 `172.18.0.1`이면 마스킹 처리.
