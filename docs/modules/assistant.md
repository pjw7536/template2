...
- SSE 답변 표시·중단·실패 재시도
- cursor 기반 대화방 검색·과거 메시지 조회와 contextKey별 장기 요약
...
2. `/emails/*`와 Observer page context가 아니면 `/api/v1/assistant/openwebui-chat/stream`으로 질문과 같은 context의 대화 이력을 보냅니다.
3. 서버가 로그인 사용자와 `knox_id`를 확인합니다.
4. 현재 대화방의 같은 `contextKey` 최근 이력을 사용합니다.
5. 같은 `contextKey`의 저장 요약과 최근 이력을 합쳐 기존 `OPENWEBUI_*` 설정으로 OpenAI 호환 Chat Completions를 호출합니다.
...
- 요약은 `(conversation, contextKey)`별 별도 row에 저장하고 같은 문맥 요청에만 주입해 Observer 조회 조건과 일반/메일 문맥을 섞거나 덮어쓰지 않습니다.
