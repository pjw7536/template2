...
### 1-5. Assistant/RAG permission_groups
핵심 정책
...
### 2-6. 외부 스냅샷 동기화
엔드포인트: `POST /api/v1/account/external-affiliations/sync` (superuser 전용)
...
## 참고 라우팅
- `apps/api/api/urls.py`에 등록된 경로 기준으로 `/api/v1/...` 노출
  - `/api/v1/account/*`
  - `/api/v1/emails/*`
  - `/api/v1/assistant/*`
