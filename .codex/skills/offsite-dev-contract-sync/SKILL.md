...
description: |
  ...
  docker-compose.dev.yml, env/api.local.env, apps/adfs_dummy 간 정합성을 유지하는 스킬.
---
...
- Django `api` 서비스는 `env/api.local.env`로 external dependency URL을 주입받는다.
...
3. dev wiring 동기화
   - `env/api.local.env`의 URL/mode flag
   - compose 서비스 env 연결
   - mock endpoint path
4. 로컬 실행 가능성 점검
   - compose 기동 가능 여부
