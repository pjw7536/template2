...
def preload_runtime_images():
    ...
    def collect(value):
        if isinstance(value, dict):
            ...
            for item in value:
                collect(item)
    collect(resources('local/portal/k8s'))
...
def portal_up(creds, restart_mock=False):
    """의존 서비스 준비 후 migration·seed를 완료하고 Portal을 실행합니다."""
    portal_secrets(creds)
    items = resources('local/portal/k8s')
...
def main():
    ...
    if args.command == 'check':
        ...
        if args.command == 'up':
            ...
            portal_up(creds, restart_mock=True)
            airflow_up(creds)
            ftp_up(creds)
            monitoring_up(creds)
            health()
            ...
            if args.app == 'portal':
                ...
                name = 'adfs' if args.app == 'mock' else args.app
                selected = [item for item in resources('local/portal/k8s') if item['metadata']['name'] == name or
                            (args.app == 'keycloak' and item['kind'] == 'ConfigMap' and item['metadata']['name'].startswith('keycloak-realm'))]
                apply(selected)
