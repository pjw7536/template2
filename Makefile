...
k8s-ui:
	KUBECTL_BIN=kubectl ./local/portal/scripts/headlamp-ui.sh
...
# 토큰 로그인 방식의 서버 Headlamp를 독립적으로 운영합니다.
HEADLAMP_ENV ?= $(CURDIR)/deploy/headlamp/env/k8s.env
.PHONY: headlamp-check headlamp-up headlamp-ui headlamp-fetch-chart
