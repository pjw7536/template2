...
Keycloak과 Airflow를 함께 운영할 때는 [서버 기동 안내](../docs/operations/server-start.md)의 `make server-up`을 사용합니다.
이 명령은 실제 Traefik의 기존 namespace 목록을 읽고 Airflow를 추가하며 먼저 권한을 준비합니다.
...
`kubectl apply -k deploy/shared/ingress`나 Keycloak 정적 스택 직접 적용은 단독 기본값을 적용하므로,
공용 앱을 연결한 서버에서는 `server-up`으로 실행합니다. 이 구분은 라이브 클러스터의 설정을 보존하기 위한 것입니다.
