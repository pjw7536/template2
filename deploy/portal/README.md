...
이전 Compose 전용 업무 주소는 실제 배포 전에 확인합니다. 예시가 실제 값의 백업은 아닙니다.

사내 서버는 Kubernetes로 배포합니다.

## 사내 배포 입력 순서

저장소 루트에서 시작합니다. 이미 만든 실제 파일은 예시로 덮어쓰지 않습니다.

```bash
# 없는 파일만 예시에서 복사하고 실제 파일의 접근 권한을 제한합니다.
for component in api web minio; do
  target="deploy/portal/env/prod/${component}.env"
  if [ ! -e "$target" ]; then
...
`env/prod/api.env.example`에 모든 번호가 보이지 않아도 정상입니다. 기존 profile의 timeout,
...
예시는 미입력 상태이므로 그대로 검사하면 실패하는 것이 정상입니다.
